...
interface Prediction {
  ...
  sig_base: string
  /** 大小球投注热度分档：如「70档 判大球 72.4」，未入档为 '' */
  ou_heat: string
  /** 前端球数倾向：判大球 / 判小球，盘口球为 '' */
  ou_pick: string
  /** 期望让球对亚盘的赢盘判断：如「客队赢盘 2.70(盘3.25)」，缺样本/缺盘口为 '' */
  exp_ah: string
  /** 期望球数综合均值的判断：如「判大球 2.85」，缺样本/走盘为 '' */
  exp_ou: string
  /** 亚盘/大小球即时盘口，无数据为 '' */
  ah_line: string
  ou_line: string
...
/** 表格总列数，日期分组行的 colspan 要跟着历史模式多出的「比分」列走 */
const columnCount = computed(() => (isHistory.value ? 16 : 14))
...
function ouClass(text: string) {
  if (text.includes('判大球')) return 'text-warning'
  if (text.includes('判小球')) return 'text-info'
  return ''
}
...
<template>
  <div>
    <div class="d-flex flex-wrap align-center mb-4 ga-3">
      <div>
        <h2 class="text-h5 font-weight-bold">终章 · 比赛预测与前瞻实测</h2>
        <div class="text-body-2 text-medium-emphasis mt-1">
          未来 {{ report?.horizon_days ?? 14 }} 天待赛比赛中命中「前端主推≥70%」信号的场次，赛前自动存档；比赛完赛后（拉回结果即可）自动逐列结算命中。
          <span class="text-error font-weight-bold">红色=偏主队</span>、<span class="text-success font-weight-bold">绿色=偏客队</span>、<span class="text-warning font-weight-bold">橙色=判大球</span>、<span class="text-info font-weight-bold">蓝色=判小球</span>；主推概率、大小球热度分档、球数倾向、期望让球、期望球数、亚盘/大小球盘口与交易/模拟盈亏推荐仅随行展示，不参与预测。<b>期望让球</b>按<b>赢盘</b>结算：拿期望让球数和亚盘线比，期望比盘口深判主队赢盘、浅判客队赢盘（如期望2.70 碰上盘口3.25，主队赢球也照样输盘），括号里是当时的亚盘线。
        </div>
      </div>
      <v-spacer />
            ...
            <tbody>
              <template v-for="group in groupedByDate" :key="group.date">
                ...
                >
                  ...
                  <td class="text-no-wrap" :class="ouClass(p.ou_heat)">
                    <span v-if="p.ou_heat">{{ p.ou_heat }}</span>
                    <span v-else class="text-medium-emphasis">-</span>
                    <span v-if="hitMark(p, 'ou_heat')" class="ml-1 font-weight-bold" :class="hitMarkClass(p, 'ou_heat')">{{ hitMark(p, 'ou_heat') }}</span>
                  </td>
                  <td class="text-no-wrap" :class="ouClass(p.ou_pick)">
                    <span v-if="p.ou_pick">{{ p.ou_pick }}</span>
                    <span v-else class="text-medium-emphasis">-</span>
                    <span v-if="hitMark(p, 'ou_pick')" class="ml-1 font-weight-bold" :class="hitMarkClass(p, 'ou_pick')">{{ hitMark(p, 'ou_pick') }}</span>
                  </td>
                  <td class="text-no-wrap" :class="sigClass(p.exp_ah)">
                    <span v-if="p.exp_ah">{{ p.exp_ah }}</span>
                    <span v-else class="text-medium-emphasis">-</span>
                    <span v-if="hitMark(p, 'exp_ah')" class="ml-1 font-weight-bold" :class="hitMarkClass(p, 'exp_ah')">{{ hitMark(p, 'exp_ah') }}</span>
                  </td>
                  <td class="text-no-wrap" :class="ouClass(p.exp_ou)">
                    <span v-if="p.exp_ou">{{ p.exp_ou }}</span>
                    <span v-else class="text-medium-emphasis">-</span>
                    <span v-if="hitMark(p, 'exp_ou')" class="ml-1 font-weight-bold" :class="hitMarkClass(p, 'exp_ou')">{{ hitMark(p, 'exp_ou') }}</span>
                  </td>
